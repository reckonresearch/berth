...
# =========================================================================
# A device with two access patterns. Not a defect: the fix for one.
# =========================================================================

def test_a_ladder_changes_the_answer_where_it_was_measured():
    """One effective bandwidth per device is the usual assumption and it is
    wrong for at least one card. At batch 16 the estimator assumed 3,445 GB/s
    where the MI300X delivers 633, and the first two proposals this agent
    generated both moved workloads onto it."""
    from berth.estimate import kv_bandwidth_gbs
    ladder = {1: 3393.0, 8: 1950.0, 16: 633.0}
    assert kv_bandwidth_gbs(ladder, 1, 3445.0) == 3393.0
    assert kv_bandwidth_gbs(ladder, 16, 3445.0) == 633.0
    # Between measured points, and held flat outside them.
    assert 633.0 < kv_bandwidth_gbs(ladder, 12, 3445.0) < 1950.0
    assert kv_bandwidth_gbs(ladder, 64, 3445.0) == 633.0


def test_no_ladder_is_algebraically_the_previous_model():
    """Every device that has not been measured this way must be unaffected,
    or a fix for one card becomes a regression for the rest."""
    from berth.estimate import kv_bandwidth_gbs
    assert kv_bandwidth_gbs(None, 32, 3445.0) == 3445.0
    assert kv_bandwidth_gbs({}, 32, 3445.0) == 3445.0


def test_a_synthetic_fleet_carries_no_ladder():
    """A ground truth is defined by the pair being recovered. A ladder in the
    generator is a third parameter the fitter cannot see, so recovery would
    fail for a reason unrelated to the estimator."""
    from berth.silicon import FLEET
    from berth.traces import make_true_fleet
    assert FLEET["mi300x"].kv_bw_ladder, "the real profile carries one"
    synthetic = make_true_fleet(FLEET, {"mi300x": (0.30, 0.50)})
    assert synthetic["mi300x"].kv_bw_ladder is None


def test_high_concurrency_no_longer_recommends_the_card_that_degrades():
    """The behaviour that mattered. At batch 16 and above the card's advantage
    is gone, and before the ladder the estimator recommended it anyway."""
    from berth.place import decide
    low = decide(workload_class="x", model_key="llama3-8b",
                 incumbent="h100-pcie", slo_bound_ms=1000, batch=4,
                 prompt_tokens=512, output_tokens=128)
    high = decide(workload_class="x", model_key="llama3-8b",
                  incumbent="h100-pcie", slo_bound_ms=5000, batch=32,
                  prompt_tokens=7680, output_tokens=256)
    assert low.recommended == "mi300x"
    assert high.recommended != "mi300x"
